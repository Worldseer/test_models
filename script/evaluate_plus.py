...
def evaluate(train_data_file, test_data_file, terms_file,
    ...
    if(export):
        export_cafa(test_df,deep_preds,"AxialGO_1_all.txt")
...
def export_cafa(test_df,deep_preds,filename='AxialGO_1_all.txt'):#all->9606 for human
    print("exporting predictions to CAFA submission format")
    txt_out=[]
        
    txt_out.append('AUTHOR AxialGO\n')
    txt_out.append('MODEL 1\n')
    txt_out.append('KEYWORDS sequence alignment.\n')
    for i, row in enumerate(test_df.itertuples()):
        prot_id = row.proteins
