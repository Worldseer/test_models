#ADAPTED FROM https://github.com/csrhddlam/axial-deeplab/blob/master/lib/models/axialnet.py
import math
import torch
...
class AxialAttention(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
        ...
        if stride > 1:
            # self.pooling = nn.MaxPool2d(stride, stride=stride)
...
class AxialAttentionNet(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=True,
                 ...
                 norm_layer=None, s=0.5,con2d_groups=8,in_dim=16,image_size=48):
        ...
        self.conv1 = nn.Conv2d(in_dim, self.inplanes, kernel_size=7, stride=2, padding=3, groups=con2d_groups, bias=False)
        # self.axial1 = AxialBlock(8, int(self.inplanes/2), stride=2, 
            # downsample=nn.Sequential(
                # conv1x1(8, self.inplanes, stride=2),
                # norm_layer(self.inplanes),
            # ), groups=8,base_width=64, dilation=1, norm_layer=None, kernel_size=48)#输出24
        # self.axial1 = self._make_layer(block, int(32 * s), 1, kernel_size=48)       
                            
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)#12
        self.layer1 = self._make_layer(block, int(128 * s), layers[0], kernel_size=int(image_size/4))
        self.layer2 = self._make_layer(block, int(256 * s), layers[1], stride=2, kernel_size=int(image_size/4),
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, int(512 * s), layers[2], stride=2, kernel_size=int(image_size/8),
...
def axial50s(pretrained=False, **kwargs):
    model = AxialAttentionNet(AxialBlock, [3, 4, 6, 3], s=0.5, num_classes=3615, **kwargs)
    return model


def axial50m(pretrained=False, **kwargs):
    model = AxialAttentionNet(AxialBlock, [3, 4, 6, 3], s=0.75, num_classes=3615 , **kwargs)
    return model


def axial50l(pretrained=False, **kwargs):
    model = AxialAttentionNet(AxialBlock, [3, 4, 6, 3], s=1, num_classes=3615, **kwargs)
    return model
